...
classdef DMPo < handle
   ...
   methods
      ...
      function [train_error, F, Fd] = train(dmp, Time, Q_data, v_rot_data, dv_rot_data, Q0, Qg0, train_method) 
...
%             g = y0*exp(-dmp.a_g*Time/tau) + g0*(1 - exp(-dmp.a_g*Time/tau));
        ...
        s = dmp.forcing_term_scaling(u, Q0, Qg0);
        if (length(s) == 1), s = ones(size(x))*s(1); end

        Fd = dmp.calc_Fd(Q_data, v_rot_data, dv_rot_data, u, Q0, Qg0, Qg);
...
%         plot(Fd')
        ...
        for k=1:3
            ...
            if (strcmpi(train_method,'LWR'))
              ...
              dmp.w{k} = LWR_train(Psi, s(k,:), Fd(k,:), dmp.zero_tol);

            elseif (strcmpi(train_method,'RLWR'))

              dmp.w{k} = RLWR_train(Psi, s(k,:), Fd(k,:));

            elseif (strcmpi(train_method,'LS'))

              dmp.w{k} = LS_train(Psi, s(k,:), Fd(k,:), dmp.zero_tol);
...
%         legend('s')
      ...
      function Fd = calc_Fd(dmp, Q, v_rot, ddv_rot, u, Q0, Qg0, Qg)
      ...
      function f_scale = forcing_term_scaling(dmp, u, Q0, Qg0)
      ...
      function shape_attr = shape_attractor(dmp, x, u, Q0, Qg0)
      ...
      function [dQ, deta] = get_states_dot(dmp, Q, eta, x, u, Q0, Qg0, Qg, Q_c, eta_c)
          ...
          v_scale = dmp.get_v_scale();
          shape_attr = dmp.shape_attractor(x, u, Q0, Qg0);
          goal_attr = dmp.goal_attractor(Q, eta, Qg);
          
          deta = ( goal_attr + shape_attr + eta_c) / v_scale;
          dQ = 0.5*quatProd([0;eta+Q_c],Q) / v_scale;
