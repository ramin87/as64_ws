...
while (true)
    ...
    for i=1:D
        ...
        dz(i) = v_scale * ( dmp{i}.a_z*(dmp{i}.b_z*(g(i)-y_robot(i))-z) + force_term ) ;
        
        dy(i) = v_scale * ( z(i) - a_py*(y_robot(i)-g(i)) + a_py*Iz(i) );
...
legend({'Force term'},'Interpreter','latex','fontsize',fontsize);
subplot(n_splots,1,3);
plot(Time,e_track_data);
legend({'$e_{track}$'},'Interpreter','latex','fontsize',fontsize);
subplot(n_splots,1,4);
plot(Time,dy_robot_data, Time,dy_data, Time,z_data, Time,Iz_data);
legend({'$\dot{y}$','$\dot{y_r}$','$z$','$I_z$'},'Interpreter','latex','fontsize',fontsize);


figure
subplot(4,1,1);
plot(Time,dy_robot_data);
legend({'$\dot{y}$'},'Interpreter','latex','fontsize',fontsize);
subplot(4,1,2);
plot(Time,dy_data);
legend({'$\dot{y_r}$'},'Interpreter','latex','fontsize',fontsize);
subplot(4,1,3);
plot(Time,z_data);
legend({'$z$'},'Interpreter','latex','fontsize',fontsize);
subplot(4,1,4);
plot(Time,Iz_data);
legend({'$I_z$'},'Interpreter','latex','fontsize',fontsize);
