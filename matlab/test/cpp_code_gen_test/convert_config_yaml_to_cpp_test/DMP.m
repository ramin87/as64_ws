...
classdef DMP < handle % : public DMP_
    properties
        N_kernels % number of kernels (basis functions) [type: int]

        a_z % parameter 'a_z' relating to the spring-damper system [type: double]
        b_z % parameter 'b_z' relating to the spring-damper system [type: double]
 
        canClockPtr % handle (pointer) to the canonical clock [type: CanonicalClock*]
        shapeAttrGatingPtr % pointer to gating function for the shape attractor [type: GatingFunction*]
        goalAttrGatingPtr % pointer to gating function for the goal attractor [type: GatingFunction*]

        w % N_kernelsx1 vector with the weights of the DMP [type: arma::vec]
        c % N_kernelsx1 vector with the kernel centers of the DMP [type: arma::vec]
        h % N_kernelsx1 vector with the kernel stds of the DMP [type: arma::vec]

        zero_tol % tolerance value used to avoid divisions with very small numbers [type: long double]

        a_s % scaling factor to ensure smaller changes in the accelaration to improve the training [type: double]

        % training params
        trainMethod % training method for weights of the DMP forcing term [type: std::string]

        lambda % forgetting factor in recursive training methods [type: double]
        P_cov% Initial value of covariance matrix in recursive training methods [type: double]

    end

    methods
        %% DMP constructor
        %  @param[in] N_kernels: the number of kernels [type: int]
        %  @param[in] a_z: Parameter 'a_z' relating to the spring-damper system. [type: double]
        %  @param[in] b_z: Parameter 'b_z' relating to the spring-damper system. [type: double]
        %  @param[in] canClockPtr: Pointer to a DMP canonical system object. [type: CanonicalClock*]
        %  @param[in] shapeAttrGatingPtr: Pointer to gating function for the shape attractor. [type: GatingFunction*]
        %  @param[in] goalAttrGatingPtr: Pointer to gating function for the goal attractor. [type: GatingFunction*]
        %  @param[in] kernelStdScaling: Scales the std of each kernel (optional, default = 1.0). [type: double]
