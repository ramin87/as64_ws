...
cmd_args.N_kernels = 40; % number of kernels used in the DMP
...
cmd_args.P_cov = 1e2; % initial value of covariance matrix for recursive training methods
...
cmd_args.M_h = 1.0;
cmd_args.K_h = 50.0;
cmd_args.D_h = 2*sqrt(cmd_args.M_h*cmd_args.K_h);

%% Robot model
cmd_args.M_r = 1.0;
cmd_args.K_r = 0.0;
cmd_args.D_r = 200; %2*sqrt(cmd_args.M_r*cmd_args.K_r);
...
cmd_args.human_load_p_var = 0.0;
cmd_args.const_wh_error = false;


%% Simulation params
cmd_args.dt = 0.008; %simulation time_step;
cmd_args.tol_stop = 0.02; % position error tolerance to stop the simulation
