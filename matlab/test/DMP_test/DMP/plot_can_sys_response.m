...
can_sysPtr = CanonicalSystem();
can_sysPtr.init(CAN_CLOCK_TYPE, CAN_FUN_TYPE, tau, u_end, u0);


%% ============================================

[x, u] = can_sysPtr.getOutput(t);
...
tau = can_sysPtr.getTau();

can_sysPtr.setTau(tau*1.0);

x = 0;
u = can_sysPtr.get_shapeVar(x);
t = 0;
...
while (abs(u-u_end)>1e-4 && x<=1)
    ...
    [dx, u]=  can_sysPtr.getOutputDot(x);
