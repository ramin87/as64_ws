...
%           figure;
%           plot(t,ddzd_data,t,g_attr_data,t,Fd);
%           legend('ddz','g-attr');
...
%               end
          ...
          F = zeros(size(Fd));
          for i=1:size(F,2)
              F(i) = dmp.shape_attractor(X(:,i),g0,y0);% / (g0-y0);
          end
...
%           error('Stop');
      ...
      function g_attr = goal_attractor(dmp,y,dy,g)
          ...
          v_scale = dmp.get_v_scale();
          g_attr = dmp.a_z*(dmp.b_z*(g-y)-dy*v_scale);% / (dmp.can_sys_ptr.tau^2 * dmp.a_s^2) ;
