...
void DMP::init(int N_kernels, double a_z, double b_z, std::shared_ptr<CanonicalSystem> cs_ptr, double std_K, bool USE_GOAL_FILT, double a_g)
{
...
void DMP::set_centers(const std::string &part_type)
{
...
double DMP::train(const arma::rowvec &yd_data, const arma::rowvec &dyd_data, const arma::rowvec &ddyd_data, double Ts, const std::string &train_method)
{
...
double DMP::train(const arma::rowvec &yd_data, const arma::rowvec &dyd_data, const arma::rowvec &ddyd_data, double Ts, const std::string &train_method, arma::rowvec &F_train, arma::rowvec &Fd_train)
{
    int n_data = yd_data.n_elem;
...
double DMP::shape_attractor(const arma::vec X, double g, double y0)
{
  double x,u;
  ...
  double s_attr = f*u*(g-y0);
    
  return s_attr;
}


double DMP::goal_attractor(double y, double dy, double g)
{
  double v_scale = this->get_v_scale();
...
double DMP::get_output(double y, double dy, double g, double y0, const arma::vec &X)
{
  double v_scale = this->get_v_scale();
...
double DMP::forcing_term(double x)
{
...
arma::vec DMP::activation_function(double x)
{
...
void DMP::set_a_z(double a)
{
  this->a_z = a;
}

void DMP::set_b_z(double a)
{
  this->b_z = a;
}

void DMP::set_tau(double t)
{
  can_sys_ptr->set_tau(t);
}

double DMP::get_a_z() const
{
  return this->a_z;
}

double DMP::get_b_z() const
{
  return this->b_z;
}
