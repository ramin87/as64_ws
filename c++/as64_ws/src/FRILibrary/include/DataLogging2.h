...
#ifndef __DataLogging2__
#define __DataLogging2__

#define AVAILABLE_READINGS_COUNT		51
#define MAX_FIELDS_DESCRIPTION_LENGTH	30
...
//  ----------------------------------------------------------
class DataLogging2 : public DataLogging
{
private:
	char Fields[3 * (AVAILABLE_READINGS_COUNT+1)][30];
	int FieldLengths[(AVAILABLE_READINGS_COUNT + 1)];
...
public:
	...
	//!
	//! \param  LoggingFileName
	//! A pointer to an array of \c char containing the description of the logging strategy
	//!	It can either be the keyword "Standard" which falls back to the typical FRI Library
	//! behovior, or a file describing which fields are to be logged
	//!
	//! \attention
	...
	//  ----------------------------------------------------------
	DataLogging2(	const char				*RobotName
...
protected:
	unsigned int ParseFields(unsigned int** GroupIndexes, unsigned int* GroupCounts);
	void WriteFieldsDescription(FILE* OutputFile);

	//  ---------------------- Doxygen info ----------------------
	//! \var LoggingStrategy
	//!
	//! \brief
	//! A pointer to an array of \c char describing the logging strategy
	//  ----------------------------------------------------------
	...
	//! A pointer to an array of \c char describing the logging strategy
	//  ----------------------------------------------------------
	unsigned int** 	FieldGroupsIndexes;
	unsigned int*  	FieldGroupsCounts;
	unsigned int 		TotalFieldsCount;
